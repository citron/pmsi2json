...
ATIH (format **RSS groupé format 122**, spécification 2025) et les convertit en JSON structuré.
...
Ces fichiers sont en format **texte à largeur fixe**, défini par la spécification
[Formats PMSI MCO 2025](https://www.atih.sante.fr/formats-pmsi-2025-0) de l'ATIH.
Ils sont **difficiles à exploiter directement** sans connaître précisément chaque position de champ.

**pmsi2json** extrait automatiquement tous les champs et produit un JSON propre, ce qui permet de :
...
Les fichiers `.grp` sont au format **RSS groupé format 122** défini par l'ATIH :

- En-tête fixe de **192 caractères** par ligne
- Puis `nDA × 8` caractères pour les diagnostics associés (CIM-10)
- Puis `nDAD × 8` caractères pour les diagnostics documentaires
- Puis `nZA × 29` caractères pour les actes CCAM

Encodage d'origine : **Windows-1252** (avec repli UTF-8 automatique).

Référence : [Formats PMSI MCO 2025 — ATIH](https://www.atih.sante.fr/formats-pmsi-2025-0)
